...
def forge_write_requested_v2_to_v3(payload: dict) -> dict:
    """forge.write.requested v2 -> v3.

    v3 adds a required (nullable) `target_external_id` — the ERP external id
    of the existing record an update modifies. A v2 event predates targeted
    updates, so it carries no target: null. (Mapper chains v1 -> v2 -> v3
    automatically for v1 producers.)
    """
    return {**payload, "target_external_id": None}
...
def register_all(mapper: Mapper) -> Mapper:
    ...
    mapper.register("forge.write.requested", 2, 3, forge_write_requested_v2_to_v3)
    return mapper
